...
module.exports.bootstrap = function (app) {
  const httpServer = config.http ? http.createServer(app) : null;
  const wsServer = config.http && config.http.ws ? expressWs(app, httpServer) : null;
  const httpsServer = config.https && config.https.tls ? createTlsServer(config.https, app) : null;
